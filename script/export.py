## copy and modify from llama2.c/export.py
...
def legacy_export_fp16(model, filepath):
    """ Original export of llama2.c bin files, i.e. version v0 """
    out_file = open(filepath, 'wb')
    ...
    # next write out the embedding weights
    serialize_fp16(out_file, model.tok_embeddings.weight)

    # now all the layers
    # attention weights
    for layer in model.layers:
        serialize_fp16(out_file, layer.attention_norm.weight)
    for layer in model.layers:
        serialize_fp16(out_file, layer.attention.wq.weight)
    for layer in model.layers:
        serialize_fp16(out_file, layer.attention.wk.weight)
    for layer in model.layers:
        serialize_fp16(out_file, layer.attention.wv.weight)
    for layer in model.layers:
        serialize_fp16(out_file, layer.attention.wo.weight)
    # ffn weights
    for layer in model.layers:
        serialize_fp16(out_file, layer.ffn_norm.weight)
    for layer in model.layers:
        serialize_fp16(out_file, layer.feed_forward.w1.weight)
    for layer in model.layers:
        serialize_fp16(out_file, layer.feed_forward.w2.weight)
    for layer in model.layers:
        serialize_fp16(out_file, layer.feed_forward.w3.weight)
    # final rmsnorm
    serialize_fp16(out_file, model.norm.weight)
    # freqs_cis
    serialize_fp16(out_file, model.freqs_cos[:p.max_seq_len])
    serialize_fp16(out_file, model.freqs_sin[:p.max_seq_len])

    # final classifier weights
    if not shared_classifier:
        serialize_fp16(out_file, model.output.weight)
...
def model_export(model, filepath, version, dtype=torch.float32):
    ...
    if version == 0:
        if dtype == torch.float32:
            ...
            legacy_export_fp16(model, filepath)
        else:
            raise ValueError(f"Unsopported dtype {dtype}") 
    elif version == 1:
