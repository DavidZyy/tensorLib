...
Tensor<dtype> originTensor(const std::vector<int>& shape, const std::string& device_type) {
    Tensor<dtype> tensor(shape, device_type);
    
    dtype *data = new dtype[tensor.num_elements];

    for(auto i=0; i<tensor.num_elements; i++)
        data[i] = static_cast<dtype>(i);

    if (device_type == "cpu") {
        std::memcpy(tensor.device->getDataPtr(), data,  tensor.num_elements * sizeof(dtype));
    } else if (device_type == "cuda") {
        CUDA_CHECK(cudaMemcpy(tensor.device->getDataPtr(), data, tensor.num_elements * sizeof(dtype), cudaMemcpyHostToDevice))
    }
...
// void test_max() {
//     Tensor<int> a = originTensor({2, 2, 2}, "cuda");
//     std::cout << "a: " << std::endl << a << std::endl;
//     auto a_max = a.max(0, true);
//     std::cout << "a_max: " << std::endl << a_max << std::endl;
// }

void test_softmax() {
    // Tensor<half> a = originTensor<half>({2, 2, 2}, "cpu");
    Tensor<float> a = originTensor<float>({2, 2, 2}, "cuda");
    // Tensor<int> a = originTensor({2, 2, 2}, "cuda");
    std::cout << "a: " << std::endl << a << std::endl;
    auto a_softmax = a.softmax(2);
    std::cout << "a_softmax: " << std::endl << a_softmax << std::endl;
}
