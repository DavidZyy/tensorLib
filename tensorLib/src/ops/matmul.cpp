...
//     }
// 
//     // make A is row major, and B is column major
//     Tensor<dtype> A = self;
//     Tensor<dtype> B = other;
// 
//     // B = B.transpose(B.ndim-2, B.ndim-1); // make B is column major
// 
...
Tensor<dtype> matmul<dtype>::call2(const Tensor<dtype> &self, const Tensor<dtype> &other) {
    ...
    // A is row major, B is column major, so in memory, A (M, K), B (N, K), C (M, N)
    int M = A.shape_[A.ndim - 2];
    int N = B.shape_[B.ndim - 2];
    int K = A.shape_[A.ndim - 1];
