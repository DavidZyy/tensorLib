...
template <typename dtype>
Tensor<dtype> Embedding<dtype>::forward(const Tensor<int> &input, std::optional<Tensor<dtype>> result_opt) const {
  auto new_shape = input.shape();
  new_shape.push_back(this->embedding_dim);
  ...
  if (result_opt.has_value()) {
    ...
    result = Tensor<dtype>(new_shape, input.device_type); // Create a new result if not passed
  }

  // embedding every element in input
  #pragma omp parallel for
  for (int i = 0; i < input.num_elements; i++) {
    auto cur_idx = input.getIndicesFromLinearIndex(i);
    int embedding_index = input.getData(cur_idx);
